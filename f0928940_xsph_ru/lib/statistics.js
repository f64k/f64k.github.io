...
(Histogram.prototype, "gmean",
 { get : function ()
   {
       return exp(this.expected(x => x == 0 ? 0 : ln(x)))
   }
 }
)
...
function Gamma(a,b)
{
    ...
    {
	var s1 = h.expected(x => x == 0 ? 0 : ln(x))
	var s = ln(h.mean) - s1,
...
{    
    ...
    res.support   = [0,1]
    res.PDF = pdf(res, x => x**(a-1)*(1-x)**(b-1)/B)
    res.CDF = cdf(res, x => betaI(a,b,x)/B)
    res.quantile = quantileFun(q => bisection(x => res.CDF(x) - q, 0, 1, x => x < 0)[0])
    res.mean      = a/(a+b)
    res.variance  = a*b/(a+b)**2/(a+b+1)
    res.mode      = undefined
    //    res.generator = () => random()*(b-a)+a
    res.fromHistogram = betaEst
    return res   
}
...
function betaEst(h) {
    const m = h.mean
    const md = h.median
    const a = m/3*(2*md-1)/(md-m)
    //var a = findRoot(x => Beta(x, x*(1-m)/m).median, a0, x => x <= md)[0]
    xx = goldenSection(x => h.bins.tail().sum(b => b < 0.05 ? (h.PMF(b)-Beta(a*x, x*a*(1-m)/m).PDF(b))**2 : 0), 0.01, 1.5)
    return Beta(a*xx, xx*a*(1-m)/m)
}
