...
function getHeadPanel() {
    var headerHtml1 = "<div style='margin:2px;'> <b>ExtJs 6.2.0</b> Оболочка отправки в REST сервис. </div>";
    ...
    var regionNorth = Ext.create('Ext.Panel', {
        ...
        dockedItems: [{
            ...
            dock: 'bottom',
            items: [
            getFileLoaderButton('Открыть Файл', processLocalFile),
            ...
            if (tabPanel) {
                ...
                if (tab) {
                    if (tab.fileToSave) {
                        ...
                        alert('НЕ РЕАЛИЗОВАНО СОХРАНЕНИЕ', tab.title);
                    }
                } else {
                    alert1("Нет активного документа на панели 'documentsTab'");
                }
            }
        }
    };
...
function getFileLoaderButton(text, onFileReady) {
    var fileLoader = new Ext.create('Ext.form.field.File', {
        buttonOnly: true,
        hideLabel: true,
        buttonText: text || 'Open File',
        listeners: {
            'render': function (t) {
...
function getTabDocFile() {
    ...
    if (tabPanel) {
        ...
        } else { Ext.MessageBox.alert("Нет активного документа на панели 'documentsTab'"); }
    } else { Ext.MessageBox.alert("Нет панели 'documentsTab'"); }
}
...
function getMenu1() {
    ...
    function onItemCheck(item, checked) {
        alert1('Item Check', Ext.String.format('You {1} the "{0}" menu item.', item.text, checked ? 'checked' : 'unchecked'));
    }

    var dateMenu = Ext.create('Ext.menu.DatePicker', {
        handler: function (dp, date) {
            alert1('Date Selected', Ext.String.format('You choose {0}.', Ext.Date.format(date, 'j M, Y')));
        }
    });
